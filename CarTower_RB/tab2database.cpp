    ...
    ui(new Ui::Tab2DataBase)
{
    ...
    sqlDb = QSqlDatabase::addDatabase("QSQLITE");
        ...
        QString query = "CREATE TABLE InOut_db (id integer primary key,date DATETIME,car VARCHAR(10), status VARCHAR(10))";
        QSqlQuery qry;
        if(!qry.exec(query))
        {
...
void Tab2DataBase:: slotTab2RecvData(QString strRecvData)
{
    ...
    QString queryData = "insert into InOut_db(date,car,status) values('"+dateTime.toString("yy/MM/dd hh:mm:ss") +"','" + car +"','" + status +"')";
    QSqlQuery qryData;
...
void Tab2DataBase:: slotSearchDb()
{
    ...
    if(qrySearch.exec(querySearch))
    {
        while (qrySearch.next()) {
            ...
            car->setText(qrySearch.value("car").toString());
            ...
            ui->pTableWidget->setItem(rowCount-1, 2, car);
